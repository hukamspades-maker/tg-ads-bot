...
class BioVerificationManager:
    ...
    @classmethod
    async def check_bio(cls, bot: Bot, user_id: int) -> tuple[bool, list[str]]:
        """Check if user's Telegram Bio contains all required tags:
        1. @SMOKEDonVIBE
        2. Bot username (@SMOKED_TGads_bot)
        Returns (passed: bool, missing_tags: list[str]).
        """
        if not REQUIRE_BIO_CHECK:
            return True, []

        tags_to_check = list(REQUIRED_BIO_TAGS) if "REQUIRED_BIO_TAGS" in globals() else ["@SMOKEDonVIBE"]
        try:
            me = await bot.get_me()
            if me.username:
                bot_tag = f"@{me.username}"
                if not any(bot_tag.lower() == t.lower() for t in tags_to_check):
                    tags_to_check.append(bot_tag)
        except Exception:
            pass
        ...
        try:
            ...
            for tag in tags_to_check:
                ...
                if tag.lower() not in bio and clean not in bio:
                    missing.append(tag)
            if not missing:
                async with cls._lock:
